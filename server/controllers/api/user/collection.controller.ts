...
class CollectionDataController extends Controller<CollectionData> {
    ...
    public create(req: Request, res: express.Response) {
        ...
        db.insert(
            new CollectionData(assign({ user_id: req.user._id }, req.body))
        );
    }
}
...
export function get_collection(req: Request, res: express.Response) {
    ...
    db.view(
        'user',
        'groups',
        { key: req.user._id, include_docs: true },
        (err, body) => {
            const assignedCollectionIds = body.rows
                .map(r => r.doc)
                .map(d => d.assigned_collections)
...
export function create_collection(req: Request, res: express.Response) {
    db.insert(
        new CollectionMeta(req.user._id, req.body.collection_id),
        (err, body) => {
            if (err) {
