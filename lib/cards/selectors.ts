...
export function select_cards_as_map(state : IState) : Map < string,
ICard > {
    return state.cards.map;
}

export function select_cards_by_ids(state : IState, card_ids : string[] = []) : ICard[] {
...
export function select_data(state : IState, user_id : string, collection_id : string, card_id : string) : IData {
    return state
        .cards
        .data
        .get(user_id + '-' + collection_id + '-' + card_id, {
            card_id,
            collection_id,
            user_id,
            _id: undefined,
