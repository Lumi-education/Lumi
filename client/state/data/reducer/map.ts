...
export default function(state: Map<string, {}> = Map<string, IData>({}), action): Map<string, {}> {
	switch (action.type) {

		case DATA_GET_SUCCESS:
			return state.merge( Map<string, {}>( arrayToObject(action.payload.data) ) );
