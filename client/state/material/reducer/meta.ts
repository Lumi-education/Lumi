...
export default function material_meta(
    state: MaterialMeta[] = initialState,
    action
): MaterialMeta[] {
    switch (action.type) {
        ...
        case MATERIAL_UPDATE_META_REQUEST:
            return state.map(
                m =>
                    m._id === action.payload.material_meta_id
                        ? assign({}, m, action.payload.update)
                        : m
            );

        case USER_INIT_SUCCESS:
            return unionBy(
                action.payload.filter(d => d.type === 'material_meta'),
                state,
                '_id'
            );
