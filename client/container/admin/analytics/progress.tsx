...
import {
	get_users_by_group
} from 'client/state/users/selectors';
import {
	select_data_as_map
} from 'client/state/data/selectors';
import {
	groups_list
} from 'client/state/groups/selectors';
// actions
import {
	get_collections,
	get_collection
...
export class AdminCollectionProgress extends React.Component<IProps, IComponentState> {
			...
			<div>
				...
				>
					<SelectField
						...
						onChange={(e, i, v) => 
							this.props.dispatch( 
								push('/admin/analytics/progress?group_id=' + v + '&collection_id=' + this.props.collection_id)
							)
						}
					>
						{this.props.groups.map(group => <MenuItem value={group._id} primaryText={group.name} />)}
					</SelectField>
					<SelectField
						...
						onChange={(e, i, v) => 
							this.props.dispatch( 
								push('/admin/analytics/progress?group_id=' + this.props.group_id + '&collection_id=' + v)
							)
						}
					>
						{this.props.collections.map(collection => <MenuItem value={collection._id} primaryText={collection.name} />)}
					</SelectField>
				</Paper>
			<Paper>
								...
								<TableHeaderColumn>Name</TableHeaderColumn>
									{ 
										this.props.cards.map((card, i) => 
											<TableHeaderColumn>{i + 1}.</TableHeaderColumn>)
									}
								...
								.map(user => {
									return (
										<TableRow>
											...
											{
												this.props.cards.map((card, i) => { 
												const data = this.props.data.filter(d => d.card_id === card._id && user._id === d.user_id)[0];
												if (!data) { return <TableRowColumn><Checkbox disabled={true} /></TableRowColumn>; }
													return (
														<TableRowColumn>
															<Checkbox 
																checkedIcon={data.score ? <SVGCheck /> : <SVGClose />} 
																checked={true} 
																iconStyle={{fill: data.score ? 'green': 'red'}}
															/>
														</TableRowColumn> 
													);
												})
												}
												{
													(() => {
														const correct_answers = this.props.data
															.filter(data => data.user_id === user._id)
															.map(data => data.score || 0)
															.reduce(sum, 0);
														const num_answers = this.props.collection.cards.length;
														const grade = (correct_answers / num_answers) * 100;
														return (
															<TableRowColumn style={{ backgroundColor: get_grade_color(grade) }}>
																{grade} %
															</TableRowColumn>
														);
													})()
...
function mapStateToProps(state: IState, ownProps): IStateProps {
	...
	return {
		...
		cards: select_cards_by_ids(state, select_collection_by_id(state, collection_id).cards ),
		collection_id: collection_id,
		group_id: group_id,
		groups: groups_list(state),
		users: get_users_by_group(state, group_id),
		data: select_data_as_map(state).toArray().filter(d => (d as any).collection_id === collection_id)
	};
}
