					...
					<Paper>
						<div style={{ display: 'flex' }}>
							...
							<div style={{ flex: 4 }}>
								<SelectField
									...
									onChange={(e, i, v) => this.setState({ card_type: v })}
								>
									<MenuItem value="text" primaryText="Markdown" />
									<MenuItem value="multiplechoice" primaryText="Multiplechoice" />
									<MenuItem value="freetext" primaryText="Freetext" />
						...
						<TagInput 
							...
							add={(tag) => { 
								...
								? 
								this.props.dispatch( add_tag_to_card(this.props.card._id, tag._id)) 
								: 
								this.props.dispatch( create_tag_and_add_to_card( this.props.card._id, tag.name ) );
							}}
							delete={(tag_id: string) => this.props.dispatch( rem_tag_from_card(this.props.card._id, tag_id))}
						/>
						<div style={{ display: 'flex' }}>
							<div style={{ flex: 6 }}>
								<TextField
								...
								<TextField
									...
									fullWidth={true}
									multiLine={true}
									onChange={(e, v) => this.setState({ items: v.split('\n') })}
								/>
								<TextField 
									hintText="Hints" 
							...
							<div style={{ flex: 6, background: 'linear-gradient(120deg, #8e44ad, #3498db)' }}>
								<MultiplechoiceCard 
						...
						<RaisedButton 
							label="Cancel" 
							style={{ margin: '10px' }} 
							onClick={() => this.props.dispatch( push('/admin/cards') )}
						/>
						<RaisedButton 
							label="Delete" 
							style={{ margin: '10px' }} 
							onClick={() => {
								this.props.dispatch( delete_card(this.props.card._id) );
								this.props.dispatch( push('/admin/cards') );
							}}
						/>
						<RaisedButton label="Duplicate" secondary={true} style={{ margin: '10px' }} />
						<RaisedButton 
							label="Save" 
							primary={true}
							style={{ margin: '10px' }}
							onClick={() => {
								this.props.dispatch( update_card(this.props.card._id, this.state) );
								this.props.dispatch( push('/admin/cards') );
							}}
						/>
					</Paper>
