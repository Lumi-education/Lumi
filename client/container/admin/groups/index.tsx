		...
		return (
			<div>
				...
				<List>
						{
							...
							.map(group => 
								...
								<div>
									<ListItem 
										leftAvatar={<Avatar>{group.name.substring(0, 3)}</Avatar>}
										primaryText={group.name} 
										onClick={() => this.props.dispatch( push('/admin/groups/' + group._id ))}
									/>
									<Divider inset={true} />
								</div>)
							)
				...
				<FloatingActionButton 
					onClick={() => this.setState({ show_create_group_dialog: true })}
				...
				{ 
					...
					? 
					<CreateGroupDialog 
						create_group={(name: string) => this.props.dispatch( create_group( name ) )}
						close={() => this.setState({ show_create_group_dialog: false })}
					/>
					: 
					null
				}
