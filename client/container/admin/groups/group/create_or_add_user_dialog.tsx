...
export class AdminCreateOrAddUserDialog extends React.Component<IProps, IComponentState> {
	...
	create_or_add_user(user) {
		if (user._id) {
			...
			this.props.dispatch( create_user(user, { groups: [ this.props.group_id ]}) );
		}
	...
	close() {
		this.props.dispatch( push( '/admin/groups/' + this.props.group_id + '/users' ));
	}
		...
		return (
			...
			>
				<AutoComplete
					...
					dataSource={this.props.users.filter(user => user.groups.indexOf( this.props.group_id ) === -1)}
