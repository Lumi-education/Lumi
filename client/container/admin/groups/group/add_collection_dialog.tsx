...
export class AdminAddCollectionDialog extends React.Component<IProps, IComponentState> {
	...
	add_collection(collection) {
		if (collection._id) {
			this.action_id = shortid();
			this.props.dispatch( add_collection_to_group( this.props.group_id, collection._id, this.action_id ) );
		} else {
	...
	close() {
		this.props.dispatch( push( '/admin/groups/' + this.props.group_id + '/collections' ));
	}
			...
			>
				<AutoComplete
					...
					dataSource={this.props.collections.filter(
							collection => this.props.group.assigned_collections.indexOf( collection._id ) === -1)
					}
